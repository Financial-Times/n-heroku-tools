...
See https://github.com/Financial-Times/n-heroku-tools/blob/master/docs/smoke.md for docs.`);
	...
	checkUrl () {
		...
		console.log(`checking ${this.name}${this.url}`); // eslint-disable-line no-console
		fetch(this.url, fetchOptions)
			.then(response => {
				...
				if (this.expected.headers) {
					let badHeaders = Object.keys(this.expected.headers).filter(function (key) {
				...
				if (this.expected.content) {
					return response
						.text()
						.then(text => {
							let errorMessage = 'bad content: ' + text;
...
function testUrls (opts) {
	const fetchers = Object.keys(opts.urls).map(function (url) {
		const test = new UrlTest({
			name: opts.name,
			url: (opts.https ? baseUrl.replace(/^http/, 'https') : baseUrl) + url,
