...
const getWebhooksUrl = appId => `https://api.heroku.com/apps/${appId}/webhooks`;
...
const repeatedCheckForReviewAppCreatedStatus = async ({ commit, reviewApp, minTimeout = MIN_TIMEOUT } = {}) => {
	...
	const checkForReviewAppCreatedStatus = async () => {
		...
		if (status === REVIEW_APP_STATUSES.pending) {
		...
		if (status === REVIEW_APP_STATUSES.created) {
			console.error(`App ${reviewApp.id} created.\nAutomatically approve CI workflow.`); // eslint-disable-line no-console
			await performFetch(process.env.WORKFLOW_APPROVAL_URL, {
				method: 'POST',
				headers: {
					'Circle-Token': process.env.CIRCLECI_TOKEN_ANTON
				}
			});
		}

		if (status === REVIEW_APP_STATUSES.creating) {
			if (!appId) {
				throw new Error(`App ${appId} being setup. Current status: ${status}`);
			} else {
				console.error(`App ${appId} is being created.\nSetup Heroku webhook with approval CI workflow url for when the app is ready.`); // eslint-disable-line no-console
				console.error(`App webhook endpoint ${getWebhooksUrl(appId)}`); // eslint-disable-line no-console
				console.error(`App webhook url ${process.env.WORKFLOW_APPROVAL_URL}`); // eslint-disable-line no-console
				await performFetch(getWebhooksUrl(appId), {
					headers: {
						...Object.assign(headers, DEFAULT_HEADERS), // resetting default headers as we dont want the Accept value for review apps (needs tidying)
						include: ['api:dyno'],
						level: 'notify',
						url: process.env.WORKFLOW_APPROVAL_URL
					}
				});
			}
		}
...
const runCheckForReviewAppCreatedStatus = async ({ commit, reviewApp, minTimeout = MIN_TIMEOUT }) => {
	...
	try {
		console.error('333-aaa'); // eslint-disable-line no-console
		const appId = await repeatedCheckForReviewAppCreatedStatus({ commit, reviewApp, minTimeout });
		console.error(`333-bbb ${appId}`); // eslint-disable-line no-console
...
const getReviewAppName = async ({ appName, repoName, branch, commit, githubToken, minTimeout = MIN_TIMEOUT }) => {
	...
	const runReviewAppCreationProcess = async () => {
		...
		if (status === CONFLICT_STATUS_CODE) {
			console.error('000'); // eslint-disable-line no-console
			...
			console.error(appName); // eslint-disable-line no-console
			return appName;

		}

		reviewApp = await response.json();
		console.error(reviewApp, null, 4); // eslint-disable-line no-console
		appId = await runCheckForReviewAppCreatedStatus({ commit, reviewApp });
