...
function makeKonstructorCall(path, options) {
	...
	console.log('http://konstructor.svc.ft.com' + path, options.headers);
	return fetch('http://konstructor.svc.ft.com' + path, options);
}
...
function open(options) {
	var payload = {
		...
		serviceIds: options.name || packageJson.name,
