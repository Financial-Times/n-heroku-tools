...
.env:
	@if $(call IS_GIT_IGNORED); then heroku auth:whoami &>/dev/null || (echo Please make sure the `Heroku CLI toolbelt` is installed (see https://toolbelt.heroku.com/). And make sure you are authenticated by running `heroku login`. && exit 1); fi
